...
public class JwtTokenUtil {
    ...
    public String generateToken(Authentication authentication, Long userId) {
        ...
        // Build and return token with user ID claim
        return Jwts.builder()
                .setSubject(authentication.getName())
                .claim(AUTHORITIES_KEY, authorities)
                .claim("userId", userId)
    ...
    public Long getUserIdFromToken(String token) {
        try {
            ...
            // Parse token and extract user ID claim
            Claims claims = Jwts.parserBuilder()
                    .setSigningKey(key)
                    .build()
                    .parseClaimsJws(token)
            ...
            // Extract user ID from claims (add this when generating token)
            ...
            log.error("Could not get user ID from token: {}", e.getMessage());
            return null;
        }
    }
} 